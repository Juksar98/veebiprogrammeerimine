...
class Photoupload {
	...
	public function resizePhoto($maxWidth, $maxHeight){
		...
		$this->myImage = $this->resizeImage($this->myTempImage, $imageWidth, $imageHeight, 
		round($imageWidth / $sizeRatio), round($imageHeight / $sizeRatio));
	}
	...
	private function resizeImage($image, $origW, $origH, $w, $h){
		...
		/*kuhu, kust,kuhu koordinaatidele x ja y, kust koordinaatidelt x ja y, kui laialt uude kohta,
		kui kõrgelt uude kohta, kui laialt võtta, kui kõrgelt võtta?*/
	...
	public function addTextWatermark($text){
		...
		imagettftext($this->myImage, 20, -30, 10, 25, $textColor, "../../graphics/ARIAL.TTF", $text);	
	}
